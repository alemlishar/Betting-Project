...
//DEBT-eliminare il componente e sostituirlo con toast e messaggi bloccanti
export const FullScreenAlert = ({
  ...
  keyboardNavigationContext.current = "dialog-recupera-biglietto"; // DEBT
  // DEBT
  useEffect(() => {
    ...
    <StyledAlertDialog onClick={closeDialog}>
      <div className={"alert"}>
        ...
        <MsgBox className={errorDesc !== undefined ? "actionBox" : ""}>
          ...
          {isActionRequired && (
            <Actions>
              <button className={"transparent"} onClick={actionDismiss}>
                No, vendi
              </button>
              <button className={"full"} onClick={actionContinue}>
                Si, disanonima
              </button>
            </Actions>
          )}
